...
def map_O(combo):
    """
    map atom_combination function into boo.
    """
    if combo == "O":
        return True
    else:
...
def concat_features_and_nmr(struc_tensor):
    """
    Combine the NMR values and features (calculated from pymatgen.structure) into one table.
    """
    table = pd.DataFrame()
    n = 0
    error_list = []
    ...
    for sample in tqdm(struc_tensor):
        try:
            ...
            nmr = nmr.loc[:, ["max_ce", "diso", "etaQ", "CQ"]]
